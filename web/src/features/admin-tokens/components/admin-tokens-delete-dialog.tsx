...
export function AdminTokensDeleteDialog() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        toast.error(result.message || t('Failed to delete token'))
      }
    } catch {
    ...
    >
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogDescription>
            {t('This will permanently delete token')}{' '}
            <span className='font-semibold'>{currentRow?.name}</span>
            {currentRow?.username
              ? ` (${t('owned by')} ${currentRow.username})`
              : ''}
