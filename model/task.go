...
type TaskPrivateData struct {
	...
	BillingSource  string `json:"billing_source,omitempty"`  // "wallet" / "subscription" / "composite"
	SubscriptionId int    `json:"subscription_id,omitempty"` // 订阅 ID，用于订阅退款
	// RequestId 是预扣时使用的幂等键，轮询阶段的补扣/退还必须复用它，
	// 否则钱包侧会退成非幂等的 quota += N。
	RequestId string `json:"request_id,omitempty"`
	// FundingSubscriptionPart / FundingWalletPart 是提交时两个资金池上的实际扣减。
	// 组合扣减的任务必须按同样的拆分做差额结算，不能整笔算到钱包头上。
	FundingSubscriptionPart int64               `json:"funding_subscription_part,omitempty"`
	FundingWalletPart       int64               `json:"funding_wallet_part,omitempty"`
