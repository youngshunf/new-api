...
func proxyTaskMedia(c *gin.Context, task *model.Task, descriptor *relaychannel.TaskContentRequest) error {
	...
	case http.StatusOK, http.StatusPartialContent, http.StatusNotModified, http.StatusRequestedRangeNotSatisfiable:
		// 状态码 200 不足以说明拿到的是产物：实测有上游（内部转发到 LiteLLM 的渠道）以
		// 200 + Content-Type: video/mp4 把 22 字节的 {"detail":"Not Found"} 原样回来。
		// 只判状态码会把这段 JSON 当视频交给客户端，客户端入桶后得到一个打不开的产物。
		// 预读一小段先判掉，再把预读内容与剩余流一起转发，避免把整个产物读进内存。
		//
		// 只对完整 200 生效：206 是分片（首片之外本来就不以 `{` 开头，判据无意义），
		// 304 无正文，HEAD 不读正文。
		stream := resp.Body
		if resp.StatusCode == http.StatusOK && c.Request.Method != http.MethodHead {
			buffered := bufio.NewReaderSize(resp.Body, videoErrorBodySniffLimit)
			head, peekErr := buffered.Peek(videoErrorBodySniffLimit)
			if (peekErr == nil || errors.Is(peekErr, io.EOF)) && looksLikeUpstreamErrorBody(head) {
				logger.LogError(c.Request.Context(), fmt.Sprintf(
					"Upstream returned an error body as task media (%d bytes): %s", len(head), string(head)))
				return &taskMediaProxyError{
					status: http.StatusBadGateway, code: "artifact_upstream_error",
					message: "Upstream returned an error body instead of artifact content",
				}
			}
			stream = io.NopCloser(buffered)
		}
